...
makeReq method url body (Format format process) = do
  res <- request (defaultRequest { method = Left method, url = url, content = body, responseFormat = format })
  pure case res of
    Right resp
      | resp.status == StatusCode 200 -> process resp.body
      | otherwise -> Left $ UnexpectedResponse resp.status
...
data Format a b
  = Format (RF.ResponseFormat a) (a -> Either AppError b)

jsonFormat :: forall a. DecodeJson a => Format Json a
jsonFormat = Format RF.json (lmap JsonError <<< decodeJson)

ignoreFormat :: Format Unit Unit
ignoreFormat = Format RF.ignore Right
